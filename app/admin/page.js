          ...
          {adminView === 'daily-ops' ? (
            ...
            <div>
              ...
              {activeTab === 'bounce-back' && (() => {
                if (!clientBounceBack || (Array.isArray(clientBounceBack) && clientBounceBack.length === 0)) return (
                  <div className="fade-in text-center py-16">
                    <span className="text-4xl mb-4 block">🎾</span>
                ...
                const entries = Array.isArray(clientBounceBack) ? clientBounceBack : [clientBounceBack]
                const BB_MODULES = [
                ...
                <div className="fade-in space-y-8">
                  {entries.map((bb, ei) => {
                    ...
                    return (
                    <div key={bb.id} className="border border-zinc-800 rounded-2xl overflow-hidden">
                      {/* Header + Score */}
                      <div className="bg-gradient-to-br from-zinc-900 via-zinc-900 to-zinc-800 border-b border-zinc-700/50 p-6 sm:p-8">
                        <div className="flex flex-col sm:flex-row items-center gap-6">
                          <div className="relative flex-shrink-0">
                          ...
                          <div className="text-center sm:text-left flex-1">
                            <h2 className="text-lg font-black text-white uppercase tracking-wider">{bb.title || 'BounceBackAbility\u2122'}</h2>
                            {bb.setback_description && <p className="text-zinc-400 text-sm mt-1 leading-relaxed">{bb.setback_description}</p>}
                            <div className="flex items-center gap-3 mt-3 flex-wrap justify-center sm:justify-start">
                              <span className={`inline-block px-3 py-1 rounded-full text-xs font-bold uppercase tracking-widest border ${
                                totalScore >= 41 ? 'bg-emerald-500/20 text-emerald-400 border-emerald-500/30' :
                                totalScore >= 31 ? 'bg-gold/20 text-gold border-gold/30' :
                              ...
                              <span className="text-zinc-600 text-xs">Module {bb.current_module || 1} of 5 &middot; Updated {bb.updated_at ? new Date(bb.updated_at).toLocaleDateString('en-GB', { day: 'numeric', month: 'short' }) : '\u2014'}</span>
                            </div>
                          </div>
                        </div>
                      </div>
                                ...
                                <div className="space-y-3">
                                  {BB_QUESTIONS[i].map((q, qi) => (
                                    <div key={qi}>
                                      ...
                                      <p className="text-zinc-300 text-sm leading-relaxed">{mod[`q${qi + 1}`] || '\u2014'}</p>
                                    </div>
                                  ))}
                                </div>
                              )}
