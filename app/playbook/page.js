...
export default function PlaybookPage() {
  ...
  const ScoreRing = ({ score, max, size = 120, strokeWidth = 8, label }) => {
    ...
    if (pct >= 0.875) color = '#C9A84C' // gold
    else if (pct >= 0.725) color = '#22c55e' // green
    else if (pct >= 0.525) color = '#eab308' // yellow
    ...
    return (
      <div>
        ...
        <div className="flex flex-col sm:flex-row gap-3 mt-8">
          ...
          <button onClick={generateAIPrompt} className="px-6 py-2.5 bg-gold text-zinc-950 rounded-lg text-xs font-bold uppercase tracking-wider hover:bg-gold-light transition flex items-center gap-2">
